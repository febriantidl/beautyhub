...
<head>
    ...
    <title>404 — Halaman Tidak Ditemukan | BeautyHub</title>
    ...
    <style>
        ...
        p { color: #7A6F6E; margin-bottom: 2rem; line-height: 1.6; }
        a { display: inline-block; padding: 0.75rem 2rem; background: linear-gradient(135deg, #D4A5A5, #9B6B6B); color: white; text-decoration: none; border-radius: 14px; font-weight: 600; transition: transform 0.2s; }
...
<body>
    <div>
        ...
        <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('mua.login') }}">← Kembali</a>
    </div>
</body>
</html>
